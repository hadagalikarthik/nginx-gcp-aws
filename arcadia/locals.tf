...
locals {
  external_name         = try(var.CLOUD_PROVIDER == "GCP" ? data.terraform_remote_state.gcp-nap[0].outputs.external_name : var.CLOUD_PROVIDER == "AWS" ? data.terraform_remote_state.aws-nap[0].outputs.external_name : null)
...
locals {
  ...
  ebs_csi_driver_role_arn = var.CLOUD_PROVIDER == "AWS" ? data.eks[0].outputs.ebs_csi_driver_role_arn : null
