...
output "external_name" {
  value = lookup(
    {
      "GCP" = try(data.kubernetes_service_v1.nginx-service.status.0.load_balancer.0.ingress.0.ip, null)
      "AWS" = try(data.kubernetes_service_v1.nginx-service.status.0.load_balancer.0.ingress.0.hostname, null)
    },
    var.CLOUD_PROVIDER,
  )
}
